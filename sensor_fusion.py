...
def readSensors(chan):
    ...
    lux = ltr.lux # Setting it here to a different value isn't passed from what's displayed
    if debug == 1:
...
try:
    while True:
        ...
        list = [dateObj.strftime("%b-%d-%Y"), timeObj.strftime("%H:%M:%S.%f"), medianT, medianH, avgLux, luxHRs]
        logWrite(list)
